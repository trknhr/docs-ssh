...
describe('helper content', () => {
  ...
  it('renders HTTPS-only skill markdown', () => {
    ...
    expect(markdown).toContain('resolves `viewer_origin` and `project`')
    expect(markdown).toContain('Require the caller or runtime to inject `DOCS_SSH_TOKEN`')
    expect(markdown).toContain('docs-ssh files list --json')
    expect(markdown).toContain("docs-ssh files search keyword --path tasks --glob '*.md' --json")
    expect(markdown).toContain('docs-ssh files write tasks/example/notes.md --input ./notes.md --json')
    expect(markdown).toContain('Do not build HTTP requests or authorization headers in the skill')
    expect(markdown.toLowerCase()).not.toContain('envvault')
    expect(markdown).not.toContain('curl ')
    expect(markdown).not.toContain('/api/v1/projects/')
